...
public class bl_PreScene : bl_PhotonHelper{
    ...
    public void LevelLoaded(int state)
    {
        ...
        else
        {
            ...
            PhotonNetwork.isMessageQueueRunning = false;
            CancelInvoke("InvokeList");
            bl_UtilityHelper.LoadLevel(PhotonNetwork.room.RoomScene());
        }
    }
    /// <summary>
    ...
    void GetStatistics()
    {
        if (TimeText != null)
        {
            int rt = (int)PhotonNetwork.room.CustomProperties[PropiertiesKeys.TimeRoomKey];
        ...
        if (MaxPlayers != null)
        {
            MaxPlayers.text = string.Format("{0}/{1}", PhotonNetwork.playerList.Length, PhotonNetwork.room.MaxPlayers);
        }
    }
    ...
    public override void OnPhotonInstantiate(PhotonMessageInfo info)
    {
        ...
        Debug.Log("This Player: " + info.sender.NickName + " is Instantiate");
    }
    /// <summary>
    /// 
    /// </summary>
