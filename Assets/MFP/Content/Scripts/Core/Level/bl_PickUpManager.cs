...
public class bl_PickUpManager : bl_PhotonHelper
{
    ...
    {
        m_View.RPC("RpcDestroyItem", PhotonTargets.All,objname.name);
    }
    ...
    public static void InstantiateItem(int SpawneableID,Vector3 position,Quaternion rotation)
    {
        ...
        m_View.RPC("RpcInstantiate", PhotonTargets.All, SpawneableID, uniqueName, position, rotation);
    }
    ...
    public void OnPickUp(string itemName,string objname, bool invokeInServer)
    {
        ...
        if (invokeInServer)
        {
            m_View.RPC("RpcInvokeListerners", PhotonTargets.Others, objname);
        }
    }
